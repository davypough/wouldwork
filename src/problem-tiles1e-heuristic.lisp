...
(define-action move-SQ
  ...
  (do (bind (loc ?tile $row $col))
      (bind (emptys $emptys))
      (if (and (assign $empty1-coord (check-coord $row (1+ $col)))
      ...
      (if (and (assign $empty1-coord (check-coord $row (1- $col)))
        ...
        (assert (assign $direction 'left)
                ...
                (emptys $new-emptys)
                (loc ?tile $row (1- $col))))
      (if (and (assign $empty1-coord (check-coord (1+ $row) $col))
               (member $empty1-coord $emptys :test #'equal))
...
(define-action move-L
  ...
  (do (bind (loc ?tile $row $col))
      ...
      (if (and (assign $empty1-coord (check-coord $row (1- $col)))
               (member $empty1-coord $emptys :test #'equal)
               (assign $empty2-coord (check-coord (1+ $row) (- $col 2)))
               (member $empty2-coord $emptys :test #'equal))
        (assert (assign $direction 'left)
                (assign $new-emptys (copy-list $emptys))
                (alexandria:deletef $new-emptys $empty1-coord :test #'equal)
      ...
      (if (and (assign $empty1-coord (check-coord (+ $row 2) $col))
               (member $empty1-coord $emptys :test #'equal)
               (assign $empty2-coord (check-coord (+ $row 2) (1- $col)))
               (member $empty2-coord $emptys :test #'equal))
        (assert (assign $direction 'down)
                (assign $new-emptys (copy-list $emptys))
                (alexandria:deletef $new-emptys $empty1-coord :test #'equal)
