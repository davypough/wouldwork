;;; Filename: problem-smallspace2.1x.lisp
...
;;; Attempt to add los occlusion of zone in area4.
;;; Extra leg to area4.


(in-package :ww)  ;required

(ww-set *problem-name* smallspace2.1x)
...
(define-update break-single-connection! (?terminus1 ?terminus2)
  (do 
      ;; Step 1: Remove bidirectional connection
      (not (connected ?terminus1 ?terminus2))
      
      ;; Step 2: Evaluate ?terminus1 for power loss
      (if (receiver ?terminus1)
        (do (bind (chroma ?terminus1 $r1-hue))
            (if (and $r1-hue
                     (not (exists (?c connector)
                            (and (connected ?c ?terminus1)
                                 (bind (color ?c $c-hue))
                                 (eql $c-hue $r1-hue)))))
              (deactivate-receiver! ?terminus1))))
      
      (if (connector ?terminus1)
        (do (bind (color ?terminus1 $conn1-color))
            (if (and $conn1-color
                     (not (or (exists (?t transmitter)
                                (and (connected ?terminus1 ?t)
                                     (bind (chroma ?t $t-hue))
                                     (eql $t-hue $conn1-color)))
                              (exists (?c connector)
                                (and (different ?c ?terminus1)
                                     (connected ?terminus1 ?c)
                                     (bind (color ?c $c-hue))
                                     (eql $c-hue $conn1-color))))))
              (chain-deactivate! ?terminus1 $conn1-color))))
      
      ;; Step 3: Evaluate ?terminus2 for power loss (symmetric to ?terminus1)
      (if (receiver ?terminus2)
        (do (bind (chroma ?terminus2 $r2-hue))
            (if (and $r2-hue
                     (not (exists (?c connector)
                            (and (connected ?c ?terminus2)
                                 (bind (color ?c $c-hue))
                                 (eql $c-hue $r2-hue)))))
              (deactivate-receiver! ?terminus2))))
      
      (if (connector ?terminus2)
        (do (bind (color ?terminus2 $conn2-color))
            (if (and $conn2-color
                     (not (or (exists (?t transmitter)
                                (and (connected ?terminus2 ?t)
                                     (bind (chroma ?t $t-hue))
                                     (eql $t-hue $conn2-color)))
                              (exists (?c connector)
                                (and (different ?c ?terminus2)
                                     (connected ?terminus2 ?c)
                                     (bind (color ?c $c-hue))
                                     (eql $c-hue $conn2-color))))))
              (chain-deactivate! ?terminus2 $conn2-color)))))
)
...
(define-action move
  ...
  (assert (loc agent1 ?area2)
          ;; Validate existing connections - break invalid ones
          (doall (?conn connector)
            (if (bind (loc ?conn $conn-area))
              (doall (?term terminus)
                (if (and (connected ?conn ?term)
                         (not (connectable $conn-area ?term)))
                  (break-single-connection! ?conn ?term)))))
          ;; NEW: Restore connections that became valid
          (doall (?conn connector)
            (if (bind (loc ?conn $conn-area))
              (doall (?term terminus)
                (if (and (not (connected ?conn ?term))
                         (connectable $conn-area ?term))
                  (do ;; Step A: Establish connection
                      (connected ?conn ?term)
                      ;; Step B: Power from ?term to ?conn
                      (setq $term-hue (get-hue-if-source ?term))
                      (if $term-hue
                        (do (bind (color ?conn $conn-color))
                            (if (or (not $conn-color) 
                                    (eql $conn-color $term-hue))
                              (chain-activate! ?conn $term-hue))))
                      ;; Step C: Re-evaluate power from ?conn
                      (bind (color ?conn $conn-color))
                      (if $conn-color
                        (chain-activate! ?conn $conn-color))))))))
)
