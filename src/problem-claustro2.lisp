;;; Filename: problem-claustro2.lisp
...
;;; Extends claustro1 to handle connectors
;;; Note: missing some los-to-fixture define-init (see claustro3)

(in-package :ww)


(ww-set *problem-name* claustro2)
...
(define-dynamic-relations
  ...
  (on (either agent cargo) $support)  ;support an agent or cargo rests on (absent if ground)
  (depressed plate)
  (open gate)
          ...
          (if (connector ?cargo)
            (do (doall (?t terminus)  ;outgoing: this connector's pairings to its termini
                ...
                (doall (?c connector)  ;incoming: other connectors paired to this one as terminus
                  (if (paired ?c ?cargo)
                    (not (paired ?c ?cargo))))))
          (if (bind (on ?cargo $support))
            (not (on ?cargo $support)))
...
#+ignore (define-action connect-connector
  ...
  ;; connector (and any relayed receiver).  Inert placement is put-cargo's job, so only
...
(define-action put-cargo
  ;; Place held cargo on the ground or on a clear support at a reachable location
      ...
      (doall (?plate plate)
        (if (and (position ?plate ?location)
          ...
          (assert (not (holding ?agent $cargo))
                  ...
                  (assign $place ?plate)
                  (finally (propagate-changes!)))))
      ;; If a box is at the drop location, one outcome rests the cargo on its clear top
      (doall (?box box)
        (if (and (location ?box ?location)
                 (clear-support-top ?box))
          (assert (not (holding ?agent $cargo))
                  ...
                  (assign $place ?box)
                  (finally (propagate-changes!)))))
      ;; And always an outcome that rests the cargo on the ground
      (assert (not (holding ?agent $cargo))
              (location $cargo ?location)
              (assign $place 'ground)
              (finally (propagate-changes!)))))
