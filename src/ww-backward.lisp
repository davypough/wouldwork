...
(defun bw-diagnose-connect-validation ()
  ...
  (let* ((gs (first (bw-corner-valid-goal-states)))
         (families '(move pickup connect))
         (selected-actions (enum-select-actions-by-family *actions* families))
         (candidates (fps-allowed-last-actions-from-state gs *actions* selected-actions
                                                          :direction :backward))
      ...
      (let ((preds (bw-regress+validate gs action :verbose nil)))
        ...
        (loop for pred in preds
          ...
          (let* ((families '(move pickup connect))
                 (selected-actions (enum-select-actions-by-family *actions* families))
                 (pred-candidates
                   (fps-allowed-last-actions-from-state pred *actions* selected-actions
                                                        :direction :backward)))
...
Traces through the regression and forward validation step by step."
  (let* ((gs (first (bw-corner-valid-goal-states))))
    ...
    (format t "~%=== Candidate actions generated by fps-allowed-last-actions-from-state ===~%")
    (let* ((families '(move pickup connect))
           (selected-actions (enum-select-actions-by-family *actions* families))
           (candidates (fps-allowed-last-actions-from-state gs *actions* selected-actions
                                                            :direction :backward)))
...
rather than the enumerated minimal goal state."
  (let ((s15 (bw-corner-state 15)))
    ...
    (let* ((families '(move pickup connect))
           (selected-actions (enum-select-actions-by-family *actions* families))
           (candidates (fps-allowed-last-actions-from-state s15 *actions* selected-actions
                                                            :direction :backward)))
      (format t "~%Candidate actions (~D):~%" (length candidates))
      (dolist (a candidates)
        (format t "  ~S~%" a))
