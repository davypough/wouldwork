;;; Filename: problem-corner-relaxed-macro1.lisp
...
;;;
;;; RELAXED + MACRO SEARCH VARIANT: merges two orthogonal efficiency approaches.
;;;
;;;   Relaxed (from problem-corner-relaxed.lisp):
;;;     Search uses base relations only; derived relations (open, active, color,
;;;     holds, beams) are NOT propagated during the search.  (open gate1) is
;;;     replaced by gate1-open-relaxed, a structural pairing-chain check over
;;;     loc/paired + static LOS.  Candidate plans are post-validated at goal
;;;     by invoking propagate-changes! and checking derived relations.
;;;
;;;   Macro (from problem-corner-macro.lisp):
;;;     Primitive pickup/move/connect are folded into macro actions
;;;     (grab-and-connect, acquire, connect-with) so one search step
;;;     covers what previously required multiple steps.
;;;
;;; The approaches are orthogonal: relaxation skips propagation per step,
;;; macros reduce step count per plan.  Combined, each search step is cheap
;;; AND there are far fewer steps to evaluate.


(in-package :ww)


(ww-set *problem-name* corner-relaxed-macro1)
...
(define-query min-steps-remaining? ()
  ;; Admissible lower bound on minimum remaining MACRO actions to reach goal.
  ;; Decomposes into two independent cost components that can be safely summed:
  ;;   1. Pairing establishment — grab-and-connect macro per missing chain
  ;;   2. Reach goal area — macro/move actions to get agent to area4
  ;;
  ;; Independence argument: cost1 counts grab-and-connect actions for goal
  ;; receiver chains (receiver2, receiver3), while cost4 counts actions to
  ;; open gate1 (via receiver1) and reach area4.  These operate on disjoint
  ;; receivers and disjoint connector resources, so they sum validly.
  ;;
  ;; RELAXED: no (active ?r) checks (derived relation unavailable during
  ;; relaxed search); gate state is approximated by gate1-open-relaxed.
  ;;
...
(define-query parameterize-beam-intersections (?intersections ?beam-geometry)
  ;; Convert intersection coordinates to full-path parameters once, so every resolution
  ;; step compares endpoints in the same coordinate system.
  (do (assign $parameterized nil)
      (ww-loop for $intersection in ?intersections do
        ...
        (if (and (>= $len1-sq 1e-20)
                 (>= $len2-sq 1e-20))
          (do (assign $vx1 (- $int-x $src1-x))
              (assign $vy1 (- $int-y $src1-y))
              (assign $vx2 (- $int-x $src2-x))
              (assign $vy2 (- $int-y $src2-y))
              (assign $t1 (/ (+ (* $vx1 $dx1) (* $vy1 $dy1)) $len1-sq))
              (assign $t2 (/ (+ (* $vx2 $dx2) (* $vy2 $dy2)) $len2-sq))
              (push (list $beam1 $beam2 $t1 $t2) $parameterized))))
      $parameterized))


(define-query compute-next-effective-t (?beams ?intersections ?obstacle-t ?current-t)
  ;; Compute every beam endpoint from the same frozen endpoint table.
  (do (assign $next-t (make-hash-table :test 'eq))
      (ww-loop for $b in ?beams do
        (do (assign $new-t (gethash $b ?obstacle-t))
            (ww-loop for $intersection in ?intersections do
              (assign $beam1 (first $intersection))
              (assign $beam2 (second $intersection))
              (if (eql $b $beam1)
                (do (assign $t-param (third $intersection))
                    (assign $blocker $beam2)
                    (assign $blocker-t (fourth $intersection)))
                (if (eql $b $beam2)
                  (do (assign $t-param (fourth $intersection))
                      (assign $blocker $beam1)
                      (assign $blocker-t (third $intersection)))
                  (assign $t-param nil)))
              (if (and $t-param
                       (>= (gethash $blocker ?current-t) $blocker-t)
                       (< $t-param $new-t))
                (assign $new-t $t-param)))
            (setf (gethash $b $next-t) $new-t)))
      $next-t))


(define-query same-effective-t (?beams ?left ?right)
  (ww-loop for $b in ?beams
           always (= (gethash $b ?left)
                     (gethash $b ?right))))


(define-query minimum-effective-t (?beams ?left ?right)
  (do (assign $minimum (make-hash-table :test 'eq))
      (ww-loop for $b in ?beams do
        (setf (gethash $b $minimum)
              (min (gethash $b ?left)
                   (gethash $b ?right))))
      $minimum))
...
(define-update update-beams-if-interference! ()
  ;; Resolve beam intersections from simultaneous endpoint snapshots.  A period-two cycle
  ;; may use the element-wise minimum of its alternating states only when that candidate is
  ;; itself a fixed point.  Any other nonconvergence is inconsistent.  Stored beam endpoints
  ;; are changed only after a fixed point has been established.
  (do
    (assign $beams (get-current-beams))
    ...
    (assign $beam-geometry (make-hash-table :test 'eq))
    (ww-loop for $b in $beams do
    ...
    (assign $intersections
            (parameterize-beam-intersections $all-intersections $beam-geometry))
    (assign $previous-t nil)
    (assign $have-previous nil)
    (assign $resolved nil)
    (ww-loop for $iteration from 1 to 10 do
      (assign $next-t
              (compute-next-effective-t
                $beams $intersections $obstacle-t $effective-t))
      (if (same-effective-t $beams $next-t $effective-t)
        (do (assign $effective-t $next-t)
            (assign $resolved t)
            (return t))
        (if (and $have-previous
                 (same-effective-t $beams $next-t $previous-t))
          (do (assign $candidate-t
                      (minimum-effective-t $beams $effective-t $next-t))
              (assign $validated-t
                      (compute-next-effective-t
                        $beams $intersections $obstacle-t $candidate-t))
              (if (same-effective-t $beams $validated-t $candidate-t)
                (do (assign $effective-t $candidate-t)
                    (assign $resolved t))
                (inconsistent-state))
              (return nil))
          (do (assign $previous-t $effective-t)
              (assign $have-previous t)
              (assign $effective-t $next-t))))
...
;;; Macro actions in relaxed discipline:
;;;   * "no cargo held" → (not (exists (?c cargo) (not (bind (loc ?c $any)))))
...
;;;   * accessible used for agent movement (exact, matches relaxed `move`);
;;;     selectable internally uses passable (relaxed adjacency)
...
(define-action grab-and-connect-with-1-terminus
  ...
  (and (not (exists (?t terminus) (paired ?connector ?t)))
       ...
       (not (exists (?c cargo) (not (bind (loc ?c $any)))))  ;base-relation equiv of no holds
...
(define-action connect-with-1-terminus
  ;; Move to destination (optional) + place held connector paired with 1 terminus.
  ;; Held connector identified via base relations: a connector with no loc.
    1
  (?agent agent ?connector connector ?t1 terminus ?area2 area)
...
(define-action connect-with-3-terminus
  ;; Move to destination (optional) + place held connector paired with 3 termini.
  ;; Held connector identified via base relations: a connector with no loc.
    1
  (?agent agent ?connector connector (combination (?t1 ?t2 ?t3) terminus) ?area2 area)
  (and (not (bind (loc ?connector $any)))  ;base-relation equiv of (holds ?agent ?connector)
       (different ?connector ?t1)
       (different ?connector ?t2)
       (different ?connector ?t3)
