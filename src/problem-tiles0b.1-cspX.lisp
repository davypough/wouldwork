;;; Filename: problem-tiles0b.1-csp.lisp
;;; Shifting Mosaic from Islands of Insight (most NW corner island)
;;; Large tiles - REVISED FOR EFFICIENCY (fail-fast validation, no sorting overhead)
(in-package :ww)
(ww-set *problem-name* tiles0b.1-csp)
...
$coords))
; NOTE: sort-coords function removed as it is no longer needed. 
; Initial lists are lexicographically sorted; removing elements preserves relative order.
...
;; FAIL-FAST VALIDATION: Short-circuits on first overlap instead of computing full difference
(every (lambda (c) (member c $empty-coords :test #'equal)) $tile-coords))
(?ref-coord)
(assert (doall (?obj object)
(do (bind (remaining ?obj $obj-coords))
;; REMOVE WITHOUT RESORTING: Removal from a sorted list preserves order of remaining elements
