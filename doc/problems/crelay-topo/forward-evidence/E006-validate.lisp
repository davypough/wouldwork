;;; Run in the same REPL after the C007 -> extraction to L4 search, before another search/stage.
(in-package :ww)

(defun forward-e006-candidate (solution index origin parent prefix)
  (let* ((segment (validate-action-sequence parent (solution.path solution)))
    ...
    (multiple-value-bind (valid diagnostic)
      ...
      (format t "~&E006-CANDIDATE-~D RECORDER=~S ~S~%" index valid diagnostic)
      (assert valid () "Candidate ~D failed cumulative recorder validation: ~S"
              index diagnostic))
    (format t "~&E006-CANDIDATE-~D SEGMENT=~S~%" index (solution.path solution))
    (format t "~&E006-CANDIDATE-~D ENDPOINT=~S~%" index signature)
    (list :candidate index :parent 'c007 :actions actions :endpoint signature)))

(defun forward-e006-validate ()
  ...
  (assert (equal *goal* '(and (has-location agent1 location4) (holding agent1 box1))))
  ...
  (assert (equalp (make-subgoal-progress-state-signature *crelay-box-start*)
                  (getf *crelay-box-checkpoint* :endpoint)))
  (with-open-file (log "doc/problems/crelay-topo/forward-evidence/E006-validation.txt"
                       :direction :output :if-exists :error :if-does-not-exist :create)
    (let ((*standard-output* (make-broadcast-stream *standard-output* log)))
      (format t "~&E006 candidates=~D; cumulative validation, no search.~%"
              (length *solution-paths*))
      (let ((records
              (loop for solution in *solution-paths* for index from 1
                    collect (forward-e006-candidate
                              solution index *crelay-box-origin* *crelay-box-start*
                              (getf *crelay-box-checkpoint* :actions)))))
        (with-open-file (stream "doc/problems/crelay-topo/forward-evidence/E006-candidates.sexp"
                                :direction :output :if-exists :error
                                :if-does-not-exist :create)
          (let ((*print-readably* t) (*print-pretty* t) (*package* (find-package :ww)))
            (write (list :problem 'crelay-topo :experiment 'e006
                         :goal '(and (has-location agent1 location4) (holding agent1 box1))
                         :origin (make-subgoal-progress-state-signature *crelay-box-origin*)
                         :candidates records) :stream stream)
            (terpri stream)))
        (format t "~&E006-ACCEPTED-CANDIDATES=~D; saved; original final goal not checked.~%"
                (length records))
        (format t "~&E006: equal endpoint signatures do not by themselves merge recorder histories.~%")
        :validated))))
