;;; One FIRST/16-thread/cutoff-10 north-box switch-access capability search.
...
(defun forward-e017-read (path)
  (with-open-file (stream path)
    (let ((*read-eval* nil) (*package* (find-package :ww)))
      (read stream))))

(defun forward-e017-write (path data)
...
(defun forward-e017-parent (origin archive checkpoint)
  ...
  (let* ((actions (getf checkpoint :actions))
    ...
    (let ((parent (action-sequence-validation-final-state replay)))
      ...
      (multiple-value-bind (valid diagnostic)
        ...
        (format t "~&E017-PARENT-C015-RECORDER=~S ~S~%" valid diagnostic)
        (assert valid))
      parent)))

(defun forward-e017-raw-candidate (solution index prefix)
  (list :candidate index :parent 'c015 :status :unvalidated
...
(defun forward-e017-replay-matches (replay signature)
...
(defun forward-e017-validate (origin parent raw)
  (let* ((record (copy-tree raw))
         ...
         (cumulative (validate-action-sequence origin (getf raw :actions)
                       :goal-test (symbol-function 'goal-fn)))
         (segment-ok (forward-e017-replay-matches segment signature))
         (cumulative-ok (forward-e017-replay-matches cumulative signature)))
    (multiple-value-bind (recorder-ok diagnostic)
      ...
      (format t "~&E017-CANDIDATE-~D STATUS=~S SEGMENT=~S CUMULATIVE=~S RECORDER=~S ~S~%"
              (getf raw :candidate) (getf record :status)
              segment-ok cumulative-ok recorder-ok diagnostic)
      record)))

(defun forward-e017-settings ()
...
(defun forward-e017-run (request-path raw-path accepted-path)
  ...
  (let* ((archive (forward-e017-read
                   "doc/problems/crelay-topo/forward-evidence/E016-candidates.sexp"))
         ...
         (settings (forward-e017-settings)))
    ...
    (let* ((parent (forward-e017-parent origin archive checkpoint))
           (parent-signature (make-subgoal-progress-state-signature parent))
           (started (get-internal-real-time)))
      (format t "~&E017-SETTINGS=~S~%E017-PARENT=~S~%" settings parent-signature)
      (forward-e017-write request-path
        (list :problem 'crelay-topo :experiment 'e017 :parent 'c015
      ...
      (let* ((elapsed (/ (- (get-internal-real-time) started)
             ...
             (raw (loop for solution in *solution-paths* for index from 1
                        collect (forward-e017-raw-candidate
                                  solution index (getf checkpoint :actions))))
             (bundle (list :problem 'crelay-topo :experiment 'e017 :parent 'c015
                           :origin (make-subgoal-progress-state-signature origin)
                           ...
                           :candidates raw)))
        (forward-e017-write raw-path bundle)
        (assert (equalp parent-signature
                        (make-subgoal-progress-state-signature *start-state*)))
        (format t "~&E017-OUTCOME=~S REASON=~S ELAPSED=~,3F RAW-CANDIDATES=~D~%"
                (getf bundle :outcome) (getf bundle :reason) elapsed (length raw))
        (finish-output)
        (if *solutions-valid*
            (setf (getf bundle :candidates)
                  (loop for row in raw collect (forward-e017-validate origin parent row)))
            (format t "E017: non-normal result; raw candidates remain unaccepted.~%"))
        (forward-e017-write accepted-path bundle)
        (format t "E017: records saved; inspect each STATUS. No final solution claimed.~%")
        (format t "E017: no further search; assess ghost-tray exchange and boarding with the live box north.~%")
        (finish-output)
        bundle))))

(defun forward-e017 (&optional
                       (stem "doc/problems/crelay-topo/forward-evidence/E017"))
  ...
  (let ((log-path (concatenate 'string stem "-output.txt"))
    ...
    (with-open-file (stream log-path :direction :output :if-exists :error
      ...
      (let ((*standard-output* (make-broadcast-stream *standard-output* stream)))
        (unwind-protect
             (forward-e017-run request-path raw-path accepted-path)
          (finish-output))))))
