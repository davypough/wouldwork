;;; One FIRST/16-thread/cutoff-10 switch-access and connector-recovery capability search.
...
(defun forward-e015-read (path)
  (with-open-file (stream path)
    (let ((*read-eval* nil) (*package* (find-package :ww)))
      (read stream))))

(defun forward-e015-write (path data)
...
(defun forward-e015-parent (origin archive checkpoint)
  ...
  (let* ((actions (getf checkpoint :actions))
    ...
    (let ((parent (action-sequence-validation-final-state replay)))
      ...
      (multiple-value-bind (valid diagnostic)
        ...
        (format t "~&E015-PARENT-C013-RECORDER=~S ~S~%" valid diagnostic)
        (assert valid))
      parent)))

(defun forward-e015-raw-candidate (solution index prefix)
  (list :candidate index :parent 'c013 :status :unvalidated
...
(defun forward-e015-replay-matches (replay signature)
...
(defun forward-e015-validate (origin parent raw)
  (let* ((record (copy-tree raw))
         ...
         (cumulative (validate-action-sequence origin (getf raw :actions)
                       :goal-test (symbol-function 'goal-fn)))
         (segment-ok (forward-e015-replay-matches segment signature))
         (cumulative-ok (forward-e015-replay-matches cumulative signature)))
    (multiple-value-bind (recorder-ok diagnostic)
      ...
      (format t "~&E015-CANDIDATE-~D STATUS=~S SEGMENT=~S CUMULATIVE=~S RECORDER=~S ~S~%"
              (getf raw :candidate) (getf record :status)
              segment-ok cumulative-ok recorder-ok diagnostic)
      record)))

(defun forward-e015-settings ()
...
(defun forward-e015-run (request-path raw-path accepted-path)
  ...
  (let* ((archive (forward-e015-read
                   "doc/problems/crelay-topo/forward-evidence/E014-candidates.sexp"))
         ...
         (settings (forward-e015-settings)))
    ...
    (let* ((parent (forward-e015-parent origin archive checkpoint))
           (parent-signature (make-subgoal-progress-state-signature parent))
           (started (get-internal-real-time)))
      (format t "~&E015-SETTINGS=~S~%E015-PARENT=~S~%" settings parent-signature)
      (forward-e015-write request-path
        (list :problem 'crelay-topo :experiment 'e015 :parent 'c013
             ...
             (open gate7) (recording-open gate5)
      ...
      (let* ((elapsed (/ (- (get-internal-real-time) started)
             ...
             (raw (loop for solution in *solution-paths* for index from 1
                        collect (forward-e015-raw-candidate
                                  solution index (getf checkpoint :actions))))
             (bundle (list :problem 'crelay-topo :experiment 'e015 :parent 'c013
                           :origin (make-subgoal-progress-state-signature origin)
                           ...
                           :candidates raw)))
        (forward-e015-write raw-path bundle)
        (assert (equalp parent-signature
                        (make-subgoal-progress-state-signature *start-state*)))
        (format t "~&E015-OUTCOME=~S REASON=~S ELAPSED=~,3F RAW-CANDIDATES=~D~%"
                (getf bundle :outcome) (getf bundle :reason) elapsed (length raw))
        (finish-output)
        (if *solutions-valid*
            (setf (getf bundle :candidates)
                  (loop for row in raw collect (forward-e015-validate origin parent row)))
            (format t "E015: non-normal result; raw candidates remain unaccepted.~%"))
        (forward-e015-write accepted-path bundle)
        (format t "E015: records saved; inspect each STATUS. No final solution claimed.~%")
        (format t "E015: no further search; assess cargo transport with ordinary gate5 closed.~%")
        (finish-output)
        bundle))))

(defun forward-e015 (&optional
                       (stem "doc/problems/crelay-topo/forward-evidence/E015"))
  "One switch-access search with reconstruction, capture and validation."
  (let ((log-path (concatenate 'string stem "-output.txt"))
    ...
    (with-open-file (stream log-path :direction :output :if-exists :error
      ...
      (let ((*standard-output* (make-broadcast-stream *standard-output* stream)))
        (unwind-protect
             (forward-e015-run request-path raw-path accepted-path)
          (finish-output))))))
