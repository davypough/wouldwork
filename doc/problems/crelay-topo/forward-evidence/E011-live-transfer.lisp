;;; One straightforward FIRST/16-thread/cutoff-4 check under standing authorization.
;;; Fresh stage CRELAY-TOPO, threads 16, cutoff 4, solution-type FIRST first.
;;; No old REPL globals, sealed solutions, serial imports, or automatic follow-up.
(in-package :ww)

(defun forward-e011-read (path)
  (with-open-file (stream path)
    (let ((*read-eval* nil) (*package* (find-package :ww)))
      (read stream))))

(defun forward-e011-write (path data)
...
(defun forward-e011-parent (origin archive checkpoint)
  ...
  (let* ((actions (getf checkpoint :actions))
    ...
    (let ((parent (action-sequence-validation-final-state replay)))
      ...
      (multiple-value-bind (valid diagnostic)
        ...
        (format t "~&E011-PARENT-C010-RECORDER=~S ~S~%" valid diagnostic)
        (assert valid))
      parent)))

(defun forward-e011-raw-candidate (solution index prefix)
  (list :candidate index :parent 'c010 :status :unvalidated
...
(defun forward-e011-replay-matches (replay signature)
...
(defun forward-e011-validate (origin parent raw)
  (let* ((record (copy-tree raw))
         ...
         (cumulative (validate-action-sequence origin (getf raw :actions)
                       :goal-test (symbol-function 'goal-fn)))
         (segment-ok (forward-e011-replay-matches segment signature))
         (cumulative-ok (forward-e011-replay-matches cumulative signature)))
    (multiple-value-bind (recorder-ok diagnostic)
      ...
      (format t "~&E011-CANDIDATE-~D STATUS=~S SEGMENT=~S CUMULATIVE=~S RECORDER=~S ~S~%"
              (getf raw :candidate) (getf record :status)
              segment-ok cumulative-ok recorder-ok diagnostic)
      record)))

(defun forward-e011-settings ()
...
(defun forward-e011-run (request-path raw-path accepted-path)
  (assert (eq *problem-name* 'crelay-topo))
  (assert (= *threads* 16))
  (assert (= *depth-cutoff* 4))
  (assert (eq *solution-type* 'first))
  (assert (null *goal-chain-session*))
  (let* ((archive (forward-e011-read
                   "doc/problems/crelay-topo/forward-evidence/E010-candidates.sexp"))
         ...
         (settings (forward-e011-settings)))
    ...
    (let* ((parent (forward-e011-parent origin archive checkpoint))
           (parent-signature (make-subgoal-progress-state-signature parent))
           (started (get-internal-real-time)))
      (format t "~&E011-SETTINGS=~S~%E011-PARENT=~S~%" settings parent-signature)
      (forward-e011-write request-path
        (list :problem 'crelay-topo :experiment 'e011 :parent 'c010
              ...
              :goal '(and (holding agent1 tray1) (on box1 tray1)
                          (on connector1 box1) (has-location agent1 location5)
      ...
      (let* ((elapsed (/ (- (get-internal-real-time) started)
             ...
             (raw (loop for solution in *solution-paths* for index from 1
                        collect (forward-e011-raw-candidate
                                  solution index (getf checkpoint :actions))))
             (bundle (list :problem 'crelay-topo :experiment 'e011 :parent 'c010
                           :origin (make-subgoal-progress-state-signature origin)
                           ...
                           :candidates raw)))
        (forward-e011-write raw-path bundle)
        (assert (equalp parent-signature
                        (make-subgoal-progress-state-signature *start-state*)))
        (format t "~&E011-OUTCOME=~S REASON=~S ELAPSED=~,3F RAW-CANDIDATES=~D~%"
                (getf bundle :outcome) (getf bundle :reason) elapsed (length raw))
        (finish-output)
        (if *solutions-valid*
            (setf (getf bundle :candidates)
                  (loop for row in raw collect (forward-e011-validate origin parent row)))
            (format t "E011: non-normal result; raw candidates remain unaccepted.~%"))
        (forward-e011-write accepted-path bundle)
        (format t "E011: records saved; inspect each STATUS. No final solution claimed.~%")
        (format t "E011: no further search; assess keeper recovery, pairings and north-access obligations.~%")
        (finish-output)
        bundle))))

(defun forward-e011 (&optional
                       (stem "doc/problems/crelay-topo/forward-evidence/E011"))
  ...
  (let ((log-path (concatenate 'string stem "-output.txt"))
    ...
    (with-open-file (stream log-path :direction :output :if-exists :error
      ...
      (let ((*standard-output* (make-broadcast-stream *standard-output* stream)))
        (unwind-protect
             (forward-e011-run request-path raw-path accepted-path)
          (finish-output))))))
