;;; One FIRST/16-thread/cutoff-10 northern box-staging capability search.
...
(defun forward-e016-read (path)
  (with-open-file (stream path)
    (let ((*read-eval* nil) (*package* (find-package :ww)))
      (read stream))))

(defun forward-e016-write (path data)
...
(defun forward-e016-parent (origin archive checkpoint)
  ...
  (let* ((actions (getf checkpoint :actions))
    ...
    (let ((parent (action-sequence-validation-final-state replay)))
      ...
      (multiple-value-bind (valid diagnostic)
        ...
        (format t "~&E016-PARENT-C013-RECORDER=~S ~S~%" valid diagnostic)
        (assert valid))
      parent)))

(defun forward-e016-raw-candidate (solution index prefix)
...
(defun forward-e016-replay-matches (replay signature)
...
(defun forward-e016-validate (origin parent raw)
  (let* ((record (copy-tree raw))
         ...
         (cumulative (validate-action-sequence origin (getf raw :actions)
                       :goal-test (symbol-function 'goal-fn)))
         (segment-ok (forward-e016-replay-matches segment signature))
         (cumulative-ok (forward-e016-replay-matches cumulative signature)))
    (multiple-value-bind (recorder-ok diagnostic)
      ...
      (format t "~&E016-CANDIDATE-~D STATUS=~S SEGMENT=~S CUMULATIVE=~S RECORDER=~S ~S~%"
              (getf raw :candidate) (getf record :status)
              segment-ok cumulative-ok recorder-ok diagnostic)
      record)))

(defun forward-e016-settings ()
...
(defun forward-e016-run (request-path raw-path accepted-path)
  ...
  (let* ((archive (forward-e016-read
         ...
         (settings (forward-e016-settings)))
    ...
    (let* ((parent (forward-e016-parent origin archive checkpoint))
           (parent-signature (make-subgoal-progress-state-signature parent))
           (started (get-internal-real-time)))
      (format t "~&E016-SETTINGS=~S~%E016-PARENT=~S~%" settings parent-signature)
      (forward-e016-write request-path
        (list :problem 'crelay-topo :experiment 'e016 :parent 'c013
             ...
             (not (switched-on switch2)) (not (recording-switched-on switch2))
      ...
      (let* ((elapsed (/ (- (get-internal-real-time) started)
             ...
             (raw (loop for solution in *solution-paths* for index from 1
                        collect (forward-e016-raw-candidate
                                  solution index (getf checkpoint :actions))))
             (bundle (list :problem 'crelay-topo :experiment 'e016 :parent 'c013
                           :origin (make-subgoal-progress-state-signature origin)
                           ...
                           :candidates raw)))
        (forward-e016-write raw-path bundle)
        (assert (equalp parent-signature
                        (make-subgoal-progress-state-signature *start-state*)))
        (format t "~&E016-OUTCOME=~S REASON=~S ELAPSED=~,3F RAW-CANDIDATES=~D~%"
                (getf bundle :outcome) (getf bundle :reason) elapsed (length raw))
        (finish-output)
        (if *solutions-valid*
            (setf (getf bundle :candidates)
                  (loop for row in raw collect (forward-e016-validate origin parent row)))
            (format t "E016: non-normal result; raw candidates remain unaccepted.~%"))
        (forward-e016-write accepted-path bundle)
        (format t "E016: records saved; inspect each STATUS. No final solution claimed.~%")
        (format t "E016: no further search; assess switch access with the live box already north.~%")
        (finish-output)
        bundle))))

(defun forward-e016 (&optional
                       (stem "doc/problems/crelay-topo/forward-evidence/E016"))
  "One northern box-staging search with reconstruction, capture and validation."
  (let ((log-path (concatenate 'string stem "-output.txt"))
    ...
    (with-open-file (stream log-path :direction :output :if-exists :error
      ...
      (let ((*standard-output* (make-broadcast-stream *standard-output* stream)))
        (unwind-protect
             (forward-e016-run request-path raw-path accepted-path)
          (finish-output))))))
