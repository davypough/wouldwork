;;; One straightforward FIRST/16-thread/cutoff-3 check under standing authorization.
;;; Fresh stage CRELAY-TOPO, threads 16, cutoff 3, solution-type FIRST first.
;;; No old REPL globals, sealed solutions, serial imports, or automatic follow-up.
(in-package :ww)

(defun forward-e009-read (path)
  (with-open-file (stream path)
    (let ((*read-eval* nil) (*package* (find-package :ww)))
      (read stream))))

(defun forward-e009-write (path data)
...
(defun forward-e009-parent (origin archive checkpoint)
  ...
  (let* ((actions (getf checkpoint :actions))
    ...
    (let ((parent (action-sequence-validation-final-state replay)))
      ...
      (multiple-value-bind (valid diagnostic)
        ...
        (format t "~&E009-PARENT-C008-RECORDER=~S ~S~%" valid diagnostic)
        (assert valid))
      parent)))

(defun forward-e009-raw-candidate (solution index prefix)
...
(defun forward-e009-replay-matches (replay signature)
...
(defun forward-e009-validate (origin parent raw)
  (let* ((record (copy-tree raw))
         ...
         (cumulative (validate-action-sequence origin (getf raw :actions)
                       :goal-test (symbol-function 'goal-fn)))
         (segment-ok (forward-e009-replay-matches segment signature))
         (cumulative-ok (forward-e009-replay-matches cumulative signature)))
    (multiple-value-bind (recorder-ok diagnostic)
      ...
      (format t "~&E009-CANDIDATE-~D STATUS=~S SEGMENT=~S CUMULATIVE=~S RECORDER=~S ~S~%"
              (getf raw :candidate) (getf record :status)
              segment-ok cumulative-ok recorder-ok diagnostic)
      record)))

(defun forward-e009-settings ()
...
(defun forward-e009-run (request-path raw-path accepted-path)
  (assert (eq *problem-name* 'crelay-topo))
  (assert (= *threads* 16))
  (assert (= *depth-cutoff* 3))
  (assert (eq *solution-type* 'first))
  (assert (null *goal-chain-session*))
  (let* ((archive (forward-e009-read
         ...
         (settings (forward-e009-settings)))
    (assert (eq (getf archive :problem) 'crelay-topo))
    (assert (equal original-goal '(has-location agent1 location19)))
    (let* ((parent (forward-e009-parent origin archive checkpoint))
           (parent-signature (make-subgoal-progress-state-signature parent))
           (started (get-internal-real-time)))
      (format t "~&E009-SETTINGS=~S~%E009-PARENT=~S~%" settings parent-signature)
      (forward-e009-write request-path
        (list :problem 'crelay-topo :experiment 'e009 :parent 'c008
              ...
              :goal '(and (on box1 tray1*) (holding agent1* tray1*)
                          ...
                          (on tray1 plate2) (on connector1 plate1)
      ...
      (let* ((elapsed (/ (- (get-internal-real-time) started)
             ...
             (raw (loop for solution in *solution-paths* for index from 1
                        collect (forward-e009-raw-candidate
                                  solution index (getf checkpoint :actions))))
             (bundle (list :problem 'crelay-topo :experiment 'e009 :parent 'c008
                           :origin (make-subgoal-progress-state-signature origin)
                           ...
                           :candidates raw)))
        (forward-e009-write raw-path bundle)
        (assert (equalp parent-signature
                        (make-subgoal-progress-state-signature *start-state*)))
        (format t "~&E009-OUTCOME=~S REASON=~S ELAPSED=~,3F RAW-CANDIDATES=~D~%"
                (getf bundle :outcome) (getf bundle :reason) elapsed (length raw))
        (finish-output)
        (if *solutions-valid*
            (setf (getf bundle :candidates)
                  (loop for row in raw collect (forward-e009-validate origin parent row)))
            (format t "E009: non-normal result; raw candidates remain unaccepted.~%"))
        (forward-e009-write accepted-path bundle)
        (format t "E009: records saved; inspect each STATUS. No final solution claimed.~%")
        (format t "E009: no further search; assess keeper, gate3 and north-access obligations.~%")
        (finish-output)
        bundle))))

(defun forward-e009 (&optional
                       (stem "doc/problems/crelay-topo/forward-evidence/E009"))
  ...
  (let ((log-path (concatenate 'string stem "-output.txt"))
    ...
    (with-open-file (stream log-path :direction :output :if-exists :error
      ...
      (let ((*standard-output* (make-broadcast-stream *standard-output* stream)))
        (unwind-protect
             (forward-e009-run request-path raw-path accepted-path)
          (finish-output))))))
