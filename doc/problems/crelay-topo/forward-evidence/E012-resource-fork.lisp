;;; Prepared proposal: one FIRST/16-thread/cutoff-12 resource-fork search.
...
(defun forward-e012-read (path)
  (with-open-file (stream path)
    (let ((*read-eval* nil) (*package* (find-package :ww)))
      (read stream))))

(defun forward-e012-write (path data)
...
(defun forward-e012-parent (origin archive checkpoint)
  ...
  (let* ((actions (getf checkpoint :actions))
    ...
    (let ((parent (action-sequence-validation-final-state replay)))
      ...
      (multiple-value-bind (valid diagnostic)
        ...
        (format t "~&E012-PARENT-C008-RECORDER=~S ~S~%" valid diagnostic)
        (assert valid))
      parent)))

(defun forward-e012-raw-candidate (solution index prefix)
...
(defun forward-e012-replay-matches (replay signature)
...
(defun forward-e012-validate (origin parent raw)
  (let* ((record (copy-tree raw))
         ...
         (cumulative (validate-action-sequence origin (getf raw :actions)
                       :goal-test (symbol-function 'goal-fn)))
         (segment-ok (forward-e012-replay-matches segment signature))
         (cumulative-ok (forward-e012-replay-matches cumulative signature)))
    (multiple-value-bind (recorder-ok diagnostic)
      ...
      (format t "~&E012-CANDIDATE-~D STATUS=~S SEGMENT=~S CUMULATIVE=~S RECORDER=~S ~S~%"
              (getf raw :candidate) (getf record :status)
              segment-ok cumulative-ok recorder-ok diagnostic)
      record)))

(defun forward-e012-settings ()
...
(defun forward-e012-run (request-path raw-path accepted-path)
  ...
  (let* ((archive (forward-e012-read
         ...
         (settings (forward-e012-settings)))
    (assert (eq (getf archive :problem) 'crelay-topo))
    (assert (equal original-goal '(has-location agent1 location19)))
    (let* ((parent (forward-e012-parent origin archive checkpoint))
           (parent-signature (make-subgoal-progress-state-signature parent))
           (started (get-internal-real-time)))
      (format t "~&E012-SETTINGS=~S~%E012-PARENT=~S~%" settings parent-signature)
      (forward-e012-write request-path
        (list :problem 'crelay-topo :experiment 'e012 :parent 'c008
      ...
      (let* ((elapsed (/ (- (get-internal-real-time) started)
             ...
             (raw (loop for solution in *solution-paths* for index from 1
                        collect (forward-e012-raw-candidate
                                  solution index (getf checkpoint :actions))))
             (bundle (list :problem 'crelay-topo :experiment 'e012 :parent 'c008
                           :origin (make-subgoal-progress-state-signature origin)
                           ...
                           :candidates raw)))
        (forward-e012-write raw-path bundle)
        (assert (equalp parent-signature
                        (make-subgoal-progress-state-signature *start-state*)))
        (format t "~&E012-OUTCOME=~S REASON=~S ELAPSED=~,3F RAW-CANDIDATES=~D~%"
                (getf bundle :outcome) (getf bundle :reason) elapsed (length raw))
        (finish-output)
        (if *solutions-valid*
            (setf (getf bundle :candidates)
                  (loop for row in raw collect (forward-e012-validate origin parent row)))
            (format t "E012: non-normal result; raw candidates remain unaccepted.~%"))
        (forward-e012-write accepted-path bundle)
        (format t "E012: records saved; inspect each STATUS. No final solution claimed.~%")
        (format t "E012: no further search; assess reload, plate3 source and northern switch schedule.~%")
        (finish-output)
        bundle))))

(defun forward-e012 (&optional
                       (stem "doc/problems/crelay-topo/forward-evidence/E012"))
  "One approved resource-fork search with reconstruction, capture and validation."
  (let ((log-path (concatenate 'string stem "-output.txt"))
    ...
    (with-open-file (stream log-path :direction :output :if-exists :error
      ...
      (let ((*standard-output* (make-broadcast-stream *standard-output* stream)))
        (unwind-protect
             (forward-e012-run request-path raw-path accepted-path)
          (finish-output))))))
