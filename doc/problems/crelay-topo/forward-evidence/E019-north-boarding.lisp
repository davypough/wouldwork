;;; One FIRST/16-thread/cutoff-10 northern ghost-tray delivery and boarding capability search.
...
(defun forward-e019-read (path)
  (with-open-file (stream path)
    (let ((*read-eval* nil) (*package* (find-package :ww)))
      (read stream))))

(defun forward-e019-write (path data)
...
(defun forward-e019-parent (origin archive checkpoint)
  ...
  (let* ((actions (getf checkpoint :actions))
    ...
    (let ((parent (action-sequence-validation-final-state replay)))
      ...
      (multiple-value-bind (valid diagnostic)
        ...
        (format t "~&E019-PARENT-C017-RECORDER=~S ~S~%" valid diagnostic)
        (assert valid))
      parent)))

(defun forward-e019-raw-candidate (solution index prefix)
  (list :candidate index :parent 'c017 :status :unvalidated
...
(defun forward-e019-replay-matches (replay signature)
...
(defun forward-e019-validate (origin parent raw)
  (let* ((record (copy-tree raw))
         ...
         (cumulative (validate-action-sequence origin (getf raw :actions)
                       :goal-test (symbol-function 'goal-fn)))
         (segment-ok (forward-e019-replay-matches segment signature))
         (cumulative-ok (forward-e019-replay-matches cumulative signature)))
    (multiple-value-bind (recorder-ok diagnostic)
      ...
      (format t "~&E019-CANDIDATE-~D STATUS=~S SEGMENT=~S CUMULATIVE=~S RECORDER=~S ~S~%"
              (getf raw :candidate) (getf record :status)
              segment-ok cumulative-ok recorder-ok diagnostic)
      record)))

(defun forward-e019-settings ()
...
(defun forward-e019-run (request-path raw-path accepted-path)
  ...
  (let* ((archive (forward-e019-read
                   "doc/problems/crelay-topo/forward-evidence/E018-candidates.sexp"))
         ...
         (settings (forward-e019-settings)))
    ...
    (let* ((parent (forward-e019-parent origin archive checkpoint))
           (parent-signature (make-subgoal-progress-state-signature parent))
           (started (get-internal-real-time)))
      (format t "~&E019-SETTINGS=~S~%E019-PARENT=~S~%" settings parent-signature)
      (forward-e019-write request-path
        (list :problem 'crelay-topo :experiment 'e019 :parent 'c017
             ...
             (holding agent1* tray1*) (has-location agent1* location12)
      ...
      (let* ((elapsed (/ (- (get-internal-real-time) started)
             ...
             (raw (loop for solution in *solution-paths* for index from 1
                        collect (forward-e019-raw-candidate
                                  solution index (getf checkpoint :actions))))
             (bundle (list :problem 'crelay-topo :experiment 'e019 :parent 'c017
                           :origin (make-subgoal-progress-state-signature origin)
                           ...
                           :candidates raw)))
        (forward-e019-write raw-path bundle)
        (assert (equalp parent-signature
                        (make-subgoal-progress-state-signature *start-state*)))
        (format t "~&E019-OUTCOME=~S REASON=~S ELAPSED=~,3F RAW-CANDIDATES=~D~%"
                (getf bundle :outcome) (getf bundle :reason) elapsed (length raw))
        (finish-output)
        (if *solutions-valid*
            (setf (getf bundle :candidates)
                  (loop for row in raw collect (forward-e019-validate origin parent row)))
            (format t "E019: non-normal result; raw candidates remain unaccepted.~%"))
        (forward-e019-write accepted-path bundle)
        (format t "E019: records saved; inspect each STATUS. No final solution claimed.~%")
        (format t "E019: no further search; assess ferry transport and source restoration from the boarded endpoint.~%")
        (finish-output)
        bundle))))

(defun forward-e019 (&optional
                       (stem "doc/problems/crelay-topo/forward-evidence/E019"))
  "One northern delivery and boarding search with reconstruction, capture and validation."
  (let ((log-path (concatenate 'string stem "-output.txt"))
    ...
    (with-open-file (stream log-path :direction :output :if-exists :error
      ...
      (let ((*standard-output* (make-broadcast-stream *standard-output* stream)))
        (unwind-protect
             (forward-e019-run request-path raw-path accepted-path)
          (finish-output))))))
