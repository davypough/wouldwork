;;; Run in the same REPL after the C003 -> L6 search, before another search/stage.
(in-package :ww)

(defun forward-e004-candidate (solution index origin parent prefix)
  (let* ((segment (validate-action-sequence parent (solution.path solution)))
    ...
    (multiple-value-bind (valid diagnostic)
      ...
      (format t "~&E004-CANDIDATE-~D RECORDER=~S ~S~%" index valid diagnostic)
      (assert valid () "Candidate ~D failed cumulative recorder validation: ~S"
              index diagnostic))
    (format t "~&E004-CANDIDATE-~D SEGMENT=~S~%" index (solution.path solution))
    (format t "~&E004-CANDIDATE-~D ENDPOINT=~S~%" index signature)
    (list :candidate index :parent 'c003 :actions actions :endpoint signature)))

(defun forward-e004-validate ()
  ...
  (assert (equal *goal* '(has-location agent1 location6)))
  ...
  (assert (equalp (make-subgoal-progress-state-signature *crelay-box-start*)
                  (getf *crelay-box-checkpoint* :endpoint)))
  (with-open-file (log "doc/problems/crelay-topo/forward-evidence/E004-validation.txt"
                       :direction :output :if-exists :error :if-does-not-exist :create)
    (let ((*standard-output* (make-broadcast-stream *standard-output* log)))
      (format t "~&E004 candidates=~D; cumulative validation, no search.~%"
              (length *solution-paths*))
      (let ((records
              (loop for solution in *solution-paths* for index from 1
                    collect (forward-e004-candidate
                              solution index *crelay-box-origin* *crelay-box-start*
                              (getf *crelay-box-checkpoint* :actions)))))
        (with-open-file (stream "doc/problems/crelay-topo/forward-evidence/E004-candidates.sexp"
                                :direction :output :if-exists :error
                                :if-does-not-exist :create)
          (let ((*print-readably* t) (*print-pretty* t) (*package* (find-package :ww)))
            (write (list :problem 'crelay-topo :experiment 'e004
                         :goal '(has-location agent1 location6)
                         :origin (make-subgoal-progress-state-signature *crelay-box-origin*)
                         :candidates records) :stream stream)
            (terpri stream)))
        (format t "~&E004-ACCEPTED-CANDIDATES=~D; saved; original final goal not checked.~%"
                (length records))
        (format t "~&E004: equal endpoint signatures do not by themselves merge recorder histories.~%")
        :validated))))
