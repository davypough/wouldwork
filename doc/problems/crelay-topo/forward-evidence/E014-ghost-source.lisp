;;; One FIRST/16-thread/cutoff-10 fresh-fork and ghost-source capability search.
...
(defun forward-e014-read (path)
  (with-open-file (stream path)
    (let ((*read-eval* nil) (*package* (find-package :ww)))
      (read stream))))

(defun forward-e014-write (path data)
...
(defun forward-e014-parent (origin archive checkpoint)
  ...
  (let* ((actions (getf checkpoint :actions))
    ...
    (let ((parent (action-sequence-validation-final-state replay)))
      ...
      (multiple-value-bind (valid diagnostic)
        ...
        (format t "~&E014-PARENT-C012-RECORDER=~S ~S~%" valid diagnostic)
        (assert valid))
      parent)))

(defun forward-e014-raw-candidate (solution index prefix)
  (list :candidate index :parent 'c012 :status :unvalidated
...
(defun forward-e014-replay-matches (replay signature)
...
(defun forward-e014-validate (origin parent raw)
  (let* ((record (copy-tree raw))
         ...
         (cumulative (validate-action-sequence origin (getf raw :actions)
                       :goal-test (symbol-function 'goal-fn)))
         (segment-ok (forward-e014-replay-matches segment signature))
         (cumulative-ok (forward-e014-replay-matches cumulative signature)))
    (multiple-value-bind (recorder-ok diagnostic)
      ...
      (format t "~&E014-CANDIDATE-~D STATUS=~S SEGMENT=~S CUMULATIVE=~S RECORDER=~S ~S~%"
              (getf raw :candidate) (getf record :status)
              segment-ok cumulative-ok recorder-ok diagnostic)
      record)))

(defun forward-e014-settings ()
...
(defun forward-e014-run (request-path raw-path accepted-path)
  ...
  (let* ((archive (forward-e014-read
                   "doc/problems/crelay-topo/forward-evidence/E013-candidates.sexp"))
         ...
         (settings (forward-e014-settings)))
    ...
    (let* ((parent (forward-e014-parent origin archive checkpoint))
           (parent-signature (make-subgoal-progress-state-signature parent))
           (started (get-internal-real-time)))
      (format t "~&E014-SETTINGS=~S~%E014-PARENT=~S~%" settings parent-signature)
      (forward-e014-write request-path
        (list :problem 'crelay-topo :experiment 'e014 :parent 'c012
              ...
              :goal '(and (on box1 plate1) (on box1* plate1)
                          (on tray1 plate2) (on tray1* plate2)
                          (holding agent1 connector1) (has-location agent1 location9)
                          (has-location connector1* location9)
                          (paired connector1* transmitter1) (paired connector1* repeater1)
                          (on agent1* plate3) (open gate4) (recording-open gate4)
                          (color connector1* red) (color repeater1 red)
                          (not (switched-on switch2))
                          (not (recording-switched-on switch2))
                          (recording-in-progress)
                          (recorder-cycles-used 3))
              :status :request-saved-before-search))
      (finish-output)
      (solve-subgoal parent
        (and (on box1 plate1) (on box1* plate1)
             (on tray1 plate2) (on tray1* plate2)
             (holding agent1 connector1) (has-location agent1 location9)
             (has-location connector1* location9)
             (paired connector1* transmitter1) (paired connector1* repeater1)
      ...
      (let* ((elapsed (/ (- (get-internal-real-time) started)
             ...
             (raw (loop for solution in *solution-paths* for index from 1
                        collect (forward-e014-raw-candidate
                                  solution index (getf checkpoint :actions))))
             (bundle (list :problem 'crelay-topo :experiment 'e014 :parent 'c012
                           :origin (make-subgoal-progress-state-signature origin)
                           ...
                           :candidates raw)))
        (forward-e014-write raw-path bundle)
        (assert (equalp parent-signature
                        (make-subgoal-progress-state-signature *start-state*)))
        (format t "~&E014-OUTCOME=~S REASON=~S ELAPSED=~,3F RAW-CANDIDATES=~D~%"
                (getf bundle :outcome) (getf bundle :reason) elapsed (length raw))
        (finish-output)
        (if *solutions-valid*
            (setf (getf bundle :candidates)
                  (loop for row in raw collect (forward-e014-validate origin parent row)))
            (format t "E014: non-normal result; raw candidates remain unaccepted.~%"))
        (forward-e014-write accepted-path bundle)
        (format t "E014: records saved; inspect each STATUS. No final solution claimed.~%")
        (format t "E014: no further search; assess live loading and northern switch resource schedule.~%")
        (finish-output)
        bundle))))

(defun forward-e014 (&optional
                       (stem "doc/problems/crelay-topo/forward-evidence/E014"))
  "One ghost-source search with reconstruction, capture and validation."
  (let ((log-path (concatenate 'string stem "-output.txt"))
    ...
    (with-open-file (stream log-path :direction :output :if-exists :error
      ...
      (let ((*standard-output* (make-broadcast-stream *standard-output* stream)))
        (unwind-protect
             (forward-e014-run request-path raw-path accepted-path)
          (finish-output))))))
