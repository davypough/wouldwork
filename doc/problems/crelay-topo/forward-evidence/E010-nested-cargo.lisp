;;; One straightforward FIRST/16-thread/cutoff-5 check under standing authorization.
;;; Fresh stage CRELAY-TOPO, threads 16, cutoff 5, solution-type FIRST first.
;;; No old REPL globals, sealed solutions, serial imports, or automatic follow-up.
(in-package :ww)

(defun forward-e010-read (path)
  (with-open-file (stream path)
    (let ((*read-eval* nil) (*package* (find-package :ww)))
      (read stream))))

(defun forward-e010-write (path data)
...
(defun forward-e010-parent (origin archive checkpoint)
  ...
  (let* ((actions (getf checkpoint :actions))
    ...
    (let ((parent (action-sequence-validation-final-state replay)))
      ...
      (multiple-value-bind (valid diagnostic)
        ...
        (format t "~&E010-PARENT-C009-RECORDER=~S ~S~%" valid diagnostic)
        (assert valid))
      parent)))

(defun forward-e010-raw-candidate (solution index prefix)
  (list :candidate index :parent 'c009 :status :unvalidated
...
(defun forward-e010-replay-matches (replay signature)
...
(defun forward-e010-validate (origin parent raw)
  (let* ((record (copy-tree raw))
         ...
         (cumulative (validate-action-sequence origin (getf raw :actions)
                       :goal-test (symbol-function 'goal-fn)))
         (segment-ok (forward-e010-replay-matches segment signature))
         (cumulative-ok (forward-e010-replay-matches cumulative signature)))
    (multiple-value-bind (recorder-ok diagnostic)
      ...
      (format t "~&E010-CANDIDATE-~D STATUS=~S SEGMENT=~S CUMULATIVE=~S RECORDER=~S ~S~%"
              (getf raw :candidate) (getf record :status)
              segment-ok cumulative-ok recorder-ok diagnostic)
      record)))

(defun forward-e010-settings ()
...
(defun forward-e010-run (request-path raw-path accepted-path)
  (assert (eq *problem-name* 'crelay-topo))
  (assert (= *threads* 16))
  (assert (= *depth-cutoff* 5))
  (assert (eq *solution-type* 'first))
  (assert (null *goal-chain-session*))
  (let* ((archive (forward-e010-read
                   "doc/problems/crelay-topo/forward-evidence/E009-candidates.sexp"))
         ...
         (settings (forward-e010-settings)))
    ...
    (let* ((parent (forward-e010-parent origin archive checkpoint))
           (parent-signature (make-subgoal-progress-state-signature parent))
           (started (get-internal-real-time)))
      (format t "~&E010-SETTINGS=~S~%E010-PARENT=~S~%" settings parent-signature)
      (forward-e010-write request-path
        (list :problem 'crelay-topo :experiment 'e010 :parent 'c009
              ...
              :goal '(and (on box1 tray1*) (holding agent1* tray1*)
                          (has-location agent1* location5)
                          (has-location agent1 location20) (blowing blower1)
                          (on tray1 plate2) (on connector1 box1)
                          (on connector1* plate1) (recording-in-progress)
      ...
      (solve-subgoal parent
        (and (on box1 tray1*)
             ...
             (has-location agent1* location5)
             (has-location agent1 location20)
             (blowing blower1)
             (on tray1 plate2)
             (on connector1 box1)
      ...
      (let* ((elapsed (/ (- (get-internal-real-time) started)
             ...
             (raw (loop for solution in *solution-paths* for index from 1
                        collect (forward-e010-raw-candidate
                                  solution index (getf checkpoint :actions))))
             (bundle (list :problem 'crelay-topo :experiment 'e010 :parent 'c009
                           :origin (make-subgoal-progress-state-signature origin)
                           ...
                           :candidates raw)))
        (forward-e010-write raw-path bundle)
        (assert (equalp parent-signature
                        (make-subgoal-progress-state-signature *start-state*)))
        (format t "~&E010-OUTCOME=~S REASON=~S ELAPSED=~,3F RAW-CANDIDATES=~D~%"
                (getf bundle :outcome) (getf bundle :reason) elapsed (length raw))
        (finish-output)
        (if *solutions-valid*
            (setf (getf bundle :candidates)
                  (loop for row in raw collect (forward-e010-validate origin parent row)))
            (format t "E010: non-normal result; raw candidates remain unaccepted.~%"))
        (forward-e010-write accepted-path bundle)
        (format t "E010: records saved; inspect each STATUS. No final solution claimed.~%")
        (format t "E010: no further search; assess live-tray transfer, keeper and north-access obligations.~%")
        (finish-output)
        bundle))))

(defun forward-e010 (&optional
                       (stem "doc/problems/crelay-topo/forward-evidence/E010"))
  "One shallow capability search with reconstruction, capture and validation."
  (let ((log-path (concatenate 'string stem "-output.txt"))
    ...
    (with-open-file (stream log-path :direction :output :if-exists :error
      ...
      (let ((*standard-output* (make-broadcast-stream *standard-output* stream)))
        (unwind-protect
             (forward-e010-run request-path raw-path accepted-path)
          (finish-output))))))
