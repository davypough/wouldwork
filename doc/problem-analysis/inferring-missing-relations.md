...
2. **Reach clears only through open gates.** `reachable-clear` (`tech/reachability.lisp`) admits a barrier only if it is a gate and open — a screen or ladder on a reach edge blocks *outright*, with no empty-handed exemption. This differs from the same obstacle on a walking traversal edge.
...
- **Hand-adding a derived fact.** The spec asserts wall, edge, or boundary geometry, so the `walking` traversal facts, `los-via`, and barrier-crossing records are computed at init. Adding one by hand fixes nothing. Fix the geometry.
